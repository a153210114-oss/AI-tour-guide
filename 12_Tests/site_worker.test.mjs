...
test("supports join, question, answer and rating flow", async () => {
  const joined = await (await call("/api/sessions/gor-live-001/join", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ locale: "zh-CN", join_token: "gor-4821-visitor" }) })).json();
...
test("visitor session requires the guide QR join credential", async () => {
  const response = await call("/api/sessions/gor-live-001/join", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ locale: "en-AU" }) });
